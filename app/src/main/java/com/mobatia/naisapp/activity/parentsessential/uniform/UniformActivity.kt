package com.mobatia.naisapp.activity.parentsessential.uniform
...
class UniformActivity : AppCompatActivity() {
    ...
    private fun callTermDatesDetailAPI() {
        ...
        val call: Call<UniformResponse> = ApiClient.getClient.uniform( 1)
        call.enqueue(object : Callback<UniformResponse> {
            override fun onFailure(call: Call<UniformResponse>, t: Throwable) {
                progress.visibility = View.GONE
            }

            override fun onResponse(
                call: Call<UniformResponse>,
                response: Response<UniformResponse>
            ) {
                progress.visibility = View.GONE
                if (response.body()!!.status == 100) {
                    val bannerImg:String=response.body()!!.data.banner_image
                    termDatesArrayList.addAll(response.body()!!.data.uniformList)
